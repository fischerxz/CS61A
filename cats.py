...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def select(text):
        goodtext = split(lower(remove_punctuation(text)))
        for word1 in goodtext:
...
def accuracy(typed, source):
    ...
    # taking care of cases when either or both lists are empty
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    #a dictionary of valid words and their diff values
    words = {}
    ...
    #find smallest diff values
    #create list of index with smallest diff, find smallest index, identify word
    mindiff = min(words.values())
    bestwords = [key for key in words if words[key] == mindiff]
    
    return word_list[min(bestwords)]
...
def feline_fixes(typed, source, limit):
    ...
    #if either empty, return len of the other
    if limit < 0:
        return 0
    if not typed:
        return len(source)
