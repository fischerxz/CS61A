...
def duplicate_link(link, val):
    ...
    if link is Link.empty:
        return link
    else: # there's items left
        if link.first == val:
            # key here is to not directly modify the existing link but create a new one and add it to the rest
            # explained by the Hint : In order to insert a link into a linked list, you need to modify the .rest of certain links
            new_node = Link(val)
            new_node.rest = link.rest
            link.rest = new_node
...
def convert_link(link):
    ...
    if link is Link.empty: 
        ...
        if isinstance(link.first,int):
            ...
            result.append(convert_link(link.first))
        #taking care of the rest
        result_rest = convert_link(link.rest)
...
def multiply_lnks(lst_of_lnks):
    ...
    """
    product = 1
    for link in lst_of_lnks: # multiply first element of every list
        if link is Link.empty: #if no element 
            return Link.empty
        product *= link.first
