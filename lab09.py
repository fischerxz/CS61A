...
def cumulative_mul(t):
    ...
    """
    #if leaf, do nothing
    #else find the total multiple 
    def helper(t,multiple):
        if t.is_leaf():
            return t.label
        else:
            multiple = t.label
            for b in t.branches:
                multiple = multiple * helper(b,1)
                t.label = multiple
            return multiple
    helper(t,1)
...
def delete(t, x):
    ...
    for b in t.branches:
        delete(b,x)
        fellow_branches = b.branches
        if b.label == x: # remove b and append its child to its parent
            if fellow_branches:
                new_branches.extend(fellow_branches)
        else:
