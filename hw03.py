...
def num_eights(n):
    ...
    # if n is 8, return 1
    #reduction to base: 8 in last digit + 8 in all other digit
    if n == 8:
        return 1
    elif n < 10 and n != 8:
        return 0 
...
def digit_distance(n):
    ...
    if n < 10:
        ...
        curr = abs((n%10)-((n//10)%10))
        return curr + digit_distance(n//10)
...
def interleaved_sum(n, odd_term, even_term):
    ...
    True
    """
    #base case: n == 1 return oddterm(1)
    #accumlation: return otherterm(n+1)
    def swap(term):
    ...
    def helper(term, current, n):
        '''return sum of all modified number between integers current and n using the function term'''
        if current > n:
            return 0
        else:
            return term(current)+ helper(swap(term),current + 1,n)
        
    return helper(odd_term,1,n)
